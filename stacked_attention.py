...
class BertClassifier(nn.Module):
    ...
    def __init__(self, args, model):  # 定义类的初始化函数，用户传入的参数
        ...
        self.multi_head_attention1 = torch.nn.MultiheadAttention(1200, 8)
        self.multi_head_attention2 = torch.nn.MultiheadAttention(1200, 8)
        self.multi_head_attention3 = torch.nn.MultiheadAttention(1200, 8)
        ...
        self.linear_r = nn.Linear(in_features=1200, out_features=1200)
    ...
    def forward(self, src, label, mask, user_c, mask_c, user_r1, user_r2, user_r3, user_s, mask_s):  # 定义forward函数，实现该模块的前向过程
        ...
        User_c = self.transformer_enconder(output_c, src_key_padding_mask=mask_c.bool())   #torch.Size([192, 8, 768])
        ...
        user_c = torch.unsqueeze(User_c, dim=1)
        user_s = torch.unsqueeze(User_s, dim=1)
        user_r = torch.stack([user_r1, user_r2, user_r3], dim=1)
        user_r = self.fr(user_r)
        user_c = user_c.transpose(0, 1)
        user_r = user_r.transpose(0, 1)
        user_s = user_s.transpose(0, 1)
        H_mpoa = H_mpoa.transpose(0, 1)

        z_output = self.sigmoid(self.linear_mpoa(H_mpoa))
        att_c = self.multi_head_attention1(user_c, H_mpoa, H_mpoa)
        att_s = self.multi_head_attention2(user_s, H_mpoa, H_mpoa)
        att_r = self.multi_head_attention3(user_r, H_mpoa, H_mpoa)
        att_c, att_s= att_c[0], att_s[0]
        att_r=att_r[0]
        z_att_c, z_att_r = self.sigmoid(
            self.linear_c(att_c)), self.sigmoid(self.linear_r(att_r))
        z_att_s = self.sigmoid(self.linear_s(att_s))
        z_c = self.sigmoid(z_output + z_att_c)
        z_r = self.sigmoid(z_output + z_att_r)
        z_s = self.sigmoid(z_output + z_att_s)
        ...
        user_s = user_s.transpose(0, 1)
        user_r = user_r.transpose(0, 1)
        user_c = user_c.reshape(self.batchsize,-1)
        user_s = user_s.reshape(self.batchsize,-1)
...
def main():
    ...
    parser.add_argument("--config_path", default="./models/config.json", type=str,
    ...
    def batch_loader(batch_size, input_ids, label_ids, mask_ids, user_c_ids, mask_c_ids,
        ...
        for i in range(instances_num // batch_size):
            ...
            yield input_ids_batch.to(device), label_ids_batch.to(device), mask_ids_batch.to(device), user_c_ids_batch.to(device), mask_c_ids_batch.to(device), user_r1_ids_batch.to(device), user_r2_ids_batch.to(device), user_r3_ids_batch.to(device), user_s_ids_batch.to(device), mask_s_ids_batch.to(device)
    ...
    def padding(tokens, mask, max_len):
        if len(tokens) > max_len:  # 判断tokens长度和args.seq_length长度大小
            tokens = tokens[:max_len]  # 截取args.seq_length长度
            mask = mask[:max_len]  # 截取args.seq_length长度
            ...
            for line_id, line in enumerate(f):
                ...
                try:
                    ...
                    if len(line) == 8:  # 如果line有3行
                        ...
                        tokens, mask = get_token(text)
                        tokens, mask = padding(tokens, mask, 36)
                        sex_tokens, mask_sex = get_token(sex)
                        address_tokens, mask_address = get_token(address)
                        flag_tokens, mask_flag = get_token(flag)
                        user_s, mask_s = padding(flag_tokens + sex_tokens[1:] + address_tokens[1:],
                                                 mask_flag + mask_sex[1:] + mask_address[1:], 36)
                        user_c, mask_c = get_token(document)
                        user_c, mask_c = padding(user_c, mask_c, 192)
                        i = int(user_id)
                        try:
                            ...
                            user_r1 = torch.tensor([0 for _ in range(64)])
                            user_r2 = torch.tensor([0 for _ in range(64)])
                            user_r3 = torch.tensor([0 for _ in range(64)])
    ...
    def evaluate(args, is_test):
        ...
        user_r1_ids = torch.tensor([sample[5].detach().numpy() for sample in dataset])
        user_r2_ids = torch.tensor([sample[6].detach().numpy() for sample in dataset])
        user_r3_ids = torch.tensor([sample[7].detach().numpy() for sample in dataset])
    ...
    user_r1_ids = torch.tensor([example[5].detach().numpy() for example in trainset])
    user_r2_ids = torch.tensor([example[6].detach().numpy() for example in trainset])
    user_r3_ids = torch.tensor([example[7].detach().numpy() for example in trainset])
